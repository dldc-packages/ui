...
export function ButtonSizeWidget({ className, ...props }: ComponentPropsWithRef<"div">) {
  const sizes: TDesignSize[] = ["6", "8", "10", "12"];
  const [highlighted, setHighlighted] = useState<TDesignSize | null>();

  function renderElem(size: TDesignSize, key?: string) {
    return (
  ...
  return (
    <div className={cn("grid grid-cols-2 gap-4", className)} {...props}>
      ...
      <HighlightedGrid
        rowsDims={sizes}
        renderCell={({ row: size, key }) => renderElem(size, key)}
